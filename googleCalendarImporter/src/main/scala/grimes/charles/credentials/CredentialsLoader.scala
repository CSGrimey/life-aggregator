...
object CredentialsLoader {
  ...
  private val credentialsUrl = uri"http://localhost:2773/systemsmanager/parameters/get"
  ...
  private def getAccessToken[F[_] : Async](serviceAccountCredsInputStream: ByteArrayInputStream): F[GoogleCredentials] =
    for {
  ...
  def load[F[_]: Async](serviceAccountCredsName: String, awsSessionToken: String, client: Client[F])
    ...
    val googleCreds = for {
      ...
      accessToken <- getAccessToken(serviceAccountCredsInputStream)
    } yield accessToken
